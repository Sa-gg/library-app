...
@section('content')
    ...
    <nav class="flex items-center gap-2 text-sm text-muted mb-6">
        ...
        <span class="text-ink font-medium">{{ __('Add New Book') }}</span>
    </nav>

    <div class="max-w-2xl">
        <h1 class="page-title mb-2">{{ __('Add New Book') }}</h1>
        <p class="text-muted text-sm mb-8">{{ __('Add a new title to the library collection.') }}</p>
        ...
        <div class="card">
            <form method="POST" action="{{ route('books.store') }}">
                ...
                <div class="p-6 sm:p-8 space-y-5">
                    ...
                    <div>
                        ...
                        <input type="text" name="title" id="title" value="{{ old('title') }}" required
                               placeholder="{{ __('e.g. The Great Gatsby') }}"
                               class="form-input">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="author" id="author" value="{{ old('author') }}" required
                               placeholder="{{ __('e.g. F. Scott Fitzgerald') }}"
                               class="form-input">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="isbn" id="isbn" value="{{ old('isbn') }}" required
                               placeholder="{{ __('e.g. 978-0-7432-7356-5') }}"
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="4"
                                  placeholder="{{ __('A brief summary of the book...') }}"
                    ...
                    <div>
                        ...
                        <p class="text-xs text-muted mt-1.5">{{ __('Number of copies in the collection.') }}</p>
                    </div>
                </div>
                ...
                <div class="px-6 sm:px-8 py-4 bg-library-50/50 border-t border-library-100 flex items-center gap-3">
                    ...
                    <a href="{{ route('books.index') }}" class="btn-ghost">{{ __('Cancel') }}</a>
                </div>
            </form>
        </div>
    </div>
