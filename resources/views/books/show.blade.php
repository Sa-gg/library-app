        ...
        <div class="card mb-6">
            <div class="p-6 sm:p-8">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 p-5 rounded-xl bg-library-50/60 border border-library-100 mb-6">
                    <div>
                        <p class="text-xs font-semibold text-library-400 uppercase tracking-wider mb-1">ISBN</p>
                        <p class="font-mono text-ink">{{ $book->isbn }}</p>
                    </div>
                    <div>
                        <p class="text-xs font-semibold text-library-400 uppercase tracking-wider mb-1">Copies</p>
                        <p class="text-ink"><span class="font-bold text-library-700">{{ $book->available }}</span> available of {{ $book->quantity }} total</p>
                    </div>
                </div>

                @if($book->description)
                    <div class="mb-6">
                        <p class="text-xs font-semibold text-library-400 uppercase tracking-wider mb-2">Description</p>
        ...
        <div class="card">
                        ...
                        <div class="px-6 py-4 flex flex-col sm:flex-row sm:items-center gap-3">
                            <div class="flex-1 min-w-0">
                                ...
                                <p class="text-xs text-muted mt-0.5">
                                    {{ $borrowing->borrowed_at->format('M d, Y') }} &mdash; Due {{ $borrowing->due_date->format('M d, Y') }}
                                </p>
                            </div>
                            @if($borrowing->returned_at)
                                <span class="badge-returned">Returned {{ $borrowing->returned_at->format('M d, Y') }}</span>
            ...
            @else
                <div class="px-6 py-10 text-center">
                    <p class="text-sm text-muted">No borrowing records for this book yet.</p>
                </div>
            @endif
        </div>
    </div>
