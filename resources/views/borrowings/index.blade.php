...
@section('title', __('All Borrowings'))

@section('content')
    {{-- Page Header --}}
    <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-8">
        <div>
            <h1 class="page-title">{{ __('Borrowings') }}</h1>
            <p class="text-muted mt-1 text-sm">{{ __('Track all active and past borrowing records.') }}</p>
        </div>
                ...
                <table class="w-full">
                    <thead>
                        <tr class="border-b border-library-100">
                            <th class="px-6 py-3.5 text-left text-xs font-semibold text-library-400 uppercase tracking-wider">{{ __('Book') }}</th>
                            <th class="px-6 py-3.5 text-left text-xs font-semibold text-library-400 uppercase tracking-wider">{{ __('Borrower') }}</th>
                            <th class="px-6 py-3.5 text-left text-xs font-semibold text-library-400 uppercase tracking-wider">{{ __('Borrowed') }}</th>
                            <th class="px-6 py-3.5 text-left text-xs font-semibold text-library-400 uppercase tracking-wider">{{ __('Due') }}</th>
                            <th class="px-6 py-3.5 text-left text-xs font-semibold text-library-400 uppercase tracking-wider">{{ __('Status') }}</th>
                            <th class="px-6 py-3.5 text-right text-xs font-semibold text-library-400 uppercase tracking-wider">{{ __('Action') }}</th>
                        </tr>
                    </thead>
                            ...
                            <tr class="hover:bg-library-50/50 transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    @if($borrowing->returned_at)
                                        <span class="badge-returned">{{ __('Returned') }}</span>
                                    ...
                                    @if(!$borrowing->returned_at)
                                        <form method="POST" action="{{ route('borrowings.return', $borrowing) }}" class="inline">
                                            ...
                                            <button type="submit"
                                                    ...
                                                    onclick="return confirm(&quot;{{ __('Mark this book as returned?') }}&quot;)">
            ...
            @foreach($borrowings as $borrowing)
                <div class="card p-4">
                    <div class="flex items-start justify-between gap-3 mb-2">
                        ...
                        @if($borrowing->returned_at)
                            <span class="badge-returned shrink-0">{{ __('Returned') }}</span>
                        @elseif($borrowing->due_date->isPast())
                            <span class="badge-overdue shrink-0">{{ __('Overdue') }}</span>
                        @else
                            <span class="badge-borrowed shrink-0">{{ __('Borrowed') }}</span>
                        @endif
                    </div>
                    <p class="text-sm text-muted">{{ $borrowing->borrower_name }}</p>
                    <p class="text-xs text-library-300 mt-1">{{ $borrowing->borrowed_at->format('M d, Y') }} &mdash; {{ __('Due') }} {{ $borrowing->due_date->format('M d, Y') }}</p>
                    @if(!$borrowing->returned_at)
    ...
    @else
        <div class="card p-12 text-center">
            ...
            <h3 class="section-title mb-1">{{ __('No borrowings yet') }}</h3>
            <p class="text-sm text-muted mb-4">{{ __('Start lending books to create borrowing records.') }}</p>
            <a href="{{ route('borrowings.create') }}" class="btn-primary">
                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4"/></svg>
                {{ __('Borrow a Book') }}
            </a>
